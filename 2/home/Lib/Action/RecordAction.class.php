...
$this->tittle='添加记录';
$conf=D('Admin');
$this->conf_frist_domain=json_decode(stripslashes($conf->getConf('frist_domain')),true);
$this->display();
}
...
$conf_important_sub_domain=json_decode(stripslashes($conf->getConf('important_sub_domain')),true);
$record=D('Record');
...
load('@.dnspod');
$conf_domain_info=json_decode(stripslashes($conf->getConf('domain_info_'.$_POST['frist_domain'])),true);
...
$this->result=array(0,"API错误！".$result['state']['message']);
}else{
$data=array(
...
load('@.dnspod');
$conf=D('Admin');
$conf_domain_info=json_decode(stripslashes($conf->getConf('domain_info_'.$record_info[1]['frist_domain'])),true);
$d=new dnspod($conf_domain_info);
...
var_dump($record->SaveRecord($data));
